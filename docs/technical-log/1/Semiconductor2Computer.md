...
集成电路让人们一直探索和寻找的快速便捷的逻辑运算能力成为可能，融合了模块化思想和通用逻辑计算的需求，计算机最重要的两个组成开始出现：微处理器（CPU）和冯·诺依曼体系结构。
...
:::
有了CPU这种可重复编程的通用计算器件之后，人们很快就发现，通过跟各种控制器结合，可以更高效操控机器；通过跟输入、输出设备结合，可以更快更方便的重复编程；通过跟存储设备结合，可以更方便的调取和保存数据。但将这些设备全部像CPU一样集成在一个电路板中，会出现功能浪费、一个模块损坏不可替换维修成本高等问题。这时模块化思想指导科学家们开始创造一种可以按需自由组建和替换的计算机；其中哈佛结构和冯·诺依曼体系结构使用最为广泛，这两种结构思想被广泛应用在嵌入式设备和现代计算机中（当然随着时代的演进已经进行了诸多改进）。由于本章介绍的内容主要为计算机，所以后面着重介绍冯·诺依曼体系结构。
...
冯·诺依曼体系结构：现代计算机的基石
模块化更好的散热，模块可替换。CPU的说法是不准确的，因为在体系结构中才存在中央处理器的概念。前文的描述只是为了容易理解，因为CPU的说法更为大众所知。

1945年，匈牙利裔美国数学家约翰·冯·诺伊曼提出了著名的冯·诺依曼体系结构。这一结构定义了计算机的基本组成部分，包括输入设备、输出设备、存储器、控制器和运算器，所有数据和指令都存储在同一个存储器中，由CPU统一管理和调度。冯·诺依曼体系结构的提出，对计算机的设计产生了深远的影响，至今仍是大多数计算机系统的标准架构。
